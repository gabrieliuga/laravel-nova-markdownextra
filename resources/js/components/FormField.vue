...
export default {
    ...
    mounted() {
        window.addEventListener("paste", (e) => {
            this.getImageFromClippy(e, (img) => {
                console.log(img)
                let formData = new FormData();
                formData.append('image', img);
                axios.post('/nova-vendor/gabrieliuga/markdownextra/imageUpload', formData).then((e)=>{
                    console.log(e);
                    if(e.data.success){
                        this.insertBefore(`![](${e.data.file.url})`,2);
                    }
                });
            });
        }, false);
    ...
    methods: {
        getImageFromClippy(e, callback){
            if(e.clipboardData === false && typeof(callback) === "function"){
                callback(undefined);
            }
            let items = e.clipboardData.items;

            if(items === undefined && typeof(callback) === "function"){
                callback(undefined);
            }
            if(items.length) {
